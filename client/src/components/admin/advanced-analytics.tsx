...
export function AdvancedAnalytics() {
  ...
  const { data: businessKPIs = {} } = useQuery<BusinessKPIs>({
  ...
  const { data: clientMetrics = {} } = useQuery<ClientMetrics>({
  ...
  const calculateAnalytics = () => {
    ...
    const monthlyData = bookings.reduce((acc: Record<string, MonthlyData>, booking: BookingData) => {
    ...
    const serviceData: ServiceBreakdown[] = services.map((service: ServiceData) => {
      const serviceBookings = bookings.filter((b: BookingData) => b.serviceId === service.id);
      const revenue = serviceBookings.reduce((sum: number, b: BookingData) => sum + (b.totalPrice || 0), 0);
      return {
        name: service.name,
    ...
    const leadSources = clients.reduce((acc: Record<string, LeadSource>, client: ClientData) => {
    ...
    Object.values(leadSources).forEach((source: LeadSource) => {
            ...
            <AreaChart data={revenueData}>
              ...
              <Tooltip 
                formatter={(value: unknown, name: string) => [
                  metricType === 'revenue' ? formatCurrency(Number(value)) : value,
