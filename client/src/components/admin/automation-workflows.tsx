...
export function AutomationWorkflows() {
  ...
  const createWorkflowMutation = useMutation({
    mutationFn: async (workflowData: unknown) => {
  ...
  const displayWorkflows: Workflow[] = workflows.length > 0 ? workflows : [
    {
      id: 2,
  ...
  const totalStats = displayWorkflows.reduce((acc, workflow) => ({
    triggered: acc.triggered + workflow.stats.triggered,
    completed: acc.completed + workflow.stats.completed,
    avgOpenRate: Math.round(displayWorkflows.reduce((sum, w) => sum + w.stats.openRate, 0) / displayWorkflows.length),
    avgClickRate: Math.round(displayWorkflows.reduce((sum, w) => sum + w.stats.clickRate, 0) / displayWorkflows.length)
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">{displayWorkflows.filter(w => w.active).length}</p>
