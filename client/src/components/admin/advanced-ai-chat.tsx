...
export function AdvancedAIChat() {
  ...
  const { data: bookingsData = [] } = useQuery<BookingData[]>({
  ...
  const { data: clientsData = [] } = useQuery<ClientData[]>({
  ...
  const { data: servicesData = [] } = useQuery<ServiceData[]>({
  ...
  const generateAIResponse = async (userMessage: string): Promise<ChatMessage> => {
    ...
    const totalRevenue = bookingsData.reduce((sum: number, booking: BookingData) => sum + (booking.totalPrice || 0), 0);
    const avgBookingValue = bookingsData.length > 0 ? totalRevenue / bookingsData.length : 0;
    const confirmedBookings = bookingsData.filter((b: BookingData) => b.status === 'confirmed').length;
    const pendingBookings = bookingsData.filter((b: BookingData) => b.status === 'pending').length;
    ...
    const servicePerformance = servicesData.map((service: ServiceData) => {
      const serviceBookings = bookingsData.filter((b: BookingData) => b.serviceId === service.id);
      const serviceRevenue = serviceBookings.reduce((sum: number, b: BookingData) => sum + (b.totalPrice || 0), 0);
      return {
        name: service.name,
