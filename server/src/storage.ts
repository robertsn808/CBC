...
// Define additional types for analytics and portal sessions
...
interface InsertClientPortalSession {
  sessionToken: string;
  clientId: number;
  expiresAt: Date;
}
...
export interface IStorage {
  ...
  getClientPortalSessions(): Promise<ClientPortalSession[]>;
  getActiveClientPortalSessions(): Promise<ClientPortalSession[]>;
  createClientPortalSession(session: InsertClientPortalSession): Promise<ClientPortalSession>;
  updateClientPortalSession(sessionToken: string, updates: Partial<ClientPortalSession>): Promise<ClientPortalSession>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getBooking(id: number): Promise<(Booking & { client: Client; service: Service }) | undefined> {
    const [result] = await this.enrichBookingQuery()
  ...
  // Client Portal Sessions
  async getClientPortalSessions(): Promise<ClientPortalSession[]> {
  return await db.select().from(clientPortalSessions).orderBy(desc(clientPortalSessions.createdAt));
  }

  async getActiveClientPortalSessions(): Promise<ClientPortalSession[]> {
  ...
  async updateClientPortalSession(sessionToken: string, updates: Partial<InsertClientPortalSession>): Promise<ClientPortalSession> {
